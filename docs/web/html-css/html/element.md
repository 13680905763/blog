...
- 网页的编码：meta 元素
  ...
  - 可以用于设置网页的字符编码，让浏览器更精准地显示每一个文字，不设置或设置错误会导致乱码
...
HTML 添加了各种各样的包含样式的 HTML 元素，strong/i/del,越加越多变了很臃肿已经分不清哪些是用来搭建基本结构，哪些用来添加样式的了,非常不方便进行管理。

后来出现了**CSS**，CSS 早起并不是只有 CSS，有很多其他方式来添加样式，只不过 CSS 最后变成一个标准，被 W3C 接管了变成一个严格的标准，每个浏览器都按照这个标准
CSS 变成标准之后就出现了**HTML 的结构和 CSS 的样式分离** 这时候开始出现两个元素 div 和 span，从一个极端（加各种 html 元素）走向另一个极端 div/span + css all in 整个页面，在很长一段时间里流行 div+css 的标准。

现在**元素语义化**，用正确的元素做正确的事 h1 p ul>li + css。
